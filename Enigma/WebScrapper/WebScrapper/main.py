...
def start_next_job():
    ...
    while True:
        ...
        Crawler.crawl_page(threading.current_thread().name, url, all_data)
        thread_queue.task_done()

def create_workers():
    for _ in range(AMOUNT_OF_THREADS):
        thread = threading.Thread(target=start_next_job)
...
Crawler(START_PAGE, DOMAIN_NAME, QUEUE_FILE, VISITED_FILE, JSON_FILE, all_data)
create_workers()
crawl()
